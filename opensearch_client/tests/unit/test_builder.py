...
DEFAULT = {"opensearch": {"client": {"hosts": ["http://127.0.0.1:9200"]}}}
...
class TestInit(TestCase):
    ...
    def test_url_schema_validation_fix(self):
        ...
        test = {"opensearch": {"client": {"hosts": ["https://127.0.0.1"]}}}
        obj = Builder(configdict=test)
    ...
    def test_url_schema_validation_raises(self):
        ...
        test = {"opensearch": {"client": {"hosts": ["127.0.0.1:9200"]}}}
        with pytest.raises(ConfigurationError):
            _ = Builder(configdict=test)
...
class TestAuth(TestCase):
    ...
    def test_pass_but_no_user(self):
        ...
        cfg["opensearch"]["other_settings"] = {}
        cfg["opensearch"]["other_settings"]["password"] = pwd
        with pytest.raises(ConfigurationError):
    ...
    def test_basic_auth_tuple(self):
        ...
        cfg = deepcopy(DEFAULT)
        cfg["opensearch"]["other_settings"] = {}
        cfg["opensearch"]["other_settings"]["username"] = usr
        cfg["opensearch"]["other_settings"]["password"] = pwd
