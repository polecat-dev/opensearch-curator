...
from opensearch_client import utils as u
...
YAML = "\n".join(["---", "opensearch:", "  client:", "    hosts: {0}"])
...
class TestEnvVars:
    ...
    def test_present(self):
        ...
        assert cfg["opensearch"]["client"]["hosts"] == os.environ.get(evar)
        del os.environ[evar]
        obj.teardown()
    ...
    def test_not_present(self):
        ...
        obj.write_config(obj.args["configfile"], YAML.format(dollar))
        cfg = u.get_yaml(obj.args["configfile"])
        assert cfg["opensearch"]["client"]["hosts"] is None
        obj.teardown()
    ...
    def test_not_present_with_default(self):
        ...
        obj.write_config(obj.args["configfile"], YAML.format(dollar))
        cfg = u.get_yaml(obj.args["configfile"])
        assert cfg["opensearch"]["client"]["hosts"] == default
        obj.teardown()
