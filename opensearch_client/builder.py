...
This module provides the `Builder` class to construct OpenSearch client connections
using configuration from dictionaries or YAML files.

Classes:
    Builder: Constructs an OpenSearch client with validated configuration.
...
class Builder:
    """
    Constructs an OpenSearch client connection from configuration.

    The `Builder` class processes configuration from a dictionary or YAML file,
    validates it, and creates an :class:`~opensearchpy.OpenSearch` client.
    It supports automatic connection and version checking, with options for
    cluster-manager-only connections. Sensitive fields are stored securely in a
    :class:`SecretStore`.

    Parameters:
        configdict (dict, optional): Configuration dictionary with an 'opensearch'
        ...
        autoconnect (bool, optional): Connect to client automatically. Defaults
            to False.
        version_min (tuple, optional): Minimum OpenSearch version as (major, minor,
            patch). Defaults to :const:`~opensearch_client.defaults.VERSION_MIN`.
        version_max (tuple, optional): Maximum OpenSearch version as
            (major, minor, patch). Defaults to :const:`~opensearch_client.defaults.VERSION_MAX`.

    Attributes:
        attributes (DotMap): Storage for configuration and settings.
        client (:class:`~opensearchpy.OpenSearch`): The OpenSearch client
            connection.
        _secrets (:class:`SecretStore`): Secure storage for sensitive fields.

    Raises:
        :exc:`~opensearch_client.exceptions.ConfigurationError`: If configuration is invalid,
            such as an invalid host schema (checked during initialization).
        :exc:`~opensearch_client.exceptions.ESClientException`: If connection to OpenSearch
            fails.
        :exc:`~opensearch_client.exceptions.NotMaster`: If `cluster_manager_only` is True and connected
    ...
    def _find_master(self) -> None:
        """
        Check if the connected node is the elected cluster manager.

        Sets is_master based on node ID comparison using
        :meth:`~opensearchpy.OpenSearch.nodes.info`.
        """
    ...
    def test_connection(self) -> ObjectApiResponse[t.Any]:
        ...
        Executes :meth:`~opensearchpy.OpenSearch.info` to verify connectivity.

        Returns:
            :class:`~elastic_transport.ObjectApiResponse`: Response from OpenSearch
                info API.
        """
