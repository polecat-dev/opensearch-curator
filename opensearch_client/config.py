...
This module provides functions to manage configuration for opensearch_client, including CLI
option setup, configuration loading from YAML files, merging CLI and config settings,
and creating OpenSearch clients. It integrates with :mod:`click` for command-line
interfaces and supports environment variables with the
:data:`~opensearch_client.defaults.ENV_VAR_PREFIX` prefix.
...
Functions:
    ...
    get_client: Create an OpenSearch client using Builder.
...
def get_client(
    ...
    """
    Create an OpenSearch client using Builder.

    Args:
        configdict (dict, optional): Configuration dictionary with 'opensearch' key.
        ...
        version_min (tuple, optional): Minimum OpenSearch version. Defaults to
            :data:`~opensearch_client.defaults.VERSION_MIN`.
        version_max (tuple, optional): Maximum OpenSearch version. Defaults to
            :data:`~opensearch_client.defaults.VERSION_MAX`.

    Returns:
        :class:`opensearchpy.OpenSearch`: Configured OpenSearch client.

    Raises:
        :exc:`~opensearch_client.exceptions.ESClientException`: If client connection fails.
