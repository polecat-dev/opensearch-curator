"""Utility functions for opensearch_client

This module provides helper functions for configuration validation, file handling, and
CLI setup in opensearch_client. It supports :class:`~opensearch_client.builder.Builder`,
:mod:`~opensearch_client.logging`, and :mod:`~opensearch_client.config` with tasks like URL validation,
YAML parsing, and sensitive data redaction.

Functions:
    check_config: Validate an OpenSearch configuration dictionary.
    ensure_list: Convert a scalar or list to a list.
    file_exists: Check if a file exists.
    get_version: Retrieve the OpenSearch version as a tuple.
...
def check_config(config: dict, quiet: bool = False) -> dict:
    """
    Validate an OpenSearch configuration dictionary.
...
def get_version(client: OpenSearch) -> t.Tuple:
    """
    Retrieve the OpenSearch version as a tuple.

    Args:
        client (:class:`opensearchpy.OpenSearch`): OpenSearch client instance.
