...
def pytest_configure(config):
    ...
    for logger_name in ['opensearch', 'urllib3', 'opensearchpy', 'elastic_transport']:
        ...
        safe_handler.setFormatter(
            logging.Formatter('%(asctime)s %(levelname)-9s %(name)22s %(funcName)22s:%(lineno)-4d %(message)s')
        )
    ...
    warnings.filterwarnings("ignore", category=DeprecationWarning, module="opensearchpy")
