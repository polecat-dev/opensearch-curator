...
There are other keys available for the `client` subkey of the `elasticsearch` root key, many of which are listed [here](https://github.com/opensearch-project/opensearch-client). The most commonly used ones (listed above) are described as follows:
